...
Script gỡ cài đặt sẽ sao lưu cấu hình, history, plugin và binary cài ở cấp user vào `~/zsh-uninstall-backup-YYYYMMDD-HHMMSS` trước khi thay đổi. Sau đó script chuyển login shell về Bash và xóa toàn bộ bundle, bao gồm Zsh, Oh My Zsh, Powerlevel10k, Starship, plugin, Git, Curl, fzf, zoxide, bat, fd và ripgrep.

> Script xóa toàn bộ bundle kể cả khi một số công cụ đã tồn tại trước khi chạy installer. Backup không được tự động khôi phục.

Chạy bằng user bình thường, **không chạy với `sudo`**:

```bash
curl -fsSL https://github.com/hiennguyen9874/zsh-setup/raw/refs/heads/main/uninstall.sh | bash
```

Nên đọc [`uninstall.sh`](uninstall.sh) trước khi chạy. Script hiển thị kế hoạch và yêu cầu xác nhận trước khi gỡ.
